...
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
    ...
    <a href="#" class="active"> <li class="nav-item" id="info"><i class="icon-info"></i><span>我的信息</span></li></a>
    ...
    <div class="title btm-line">我的信息</div>
        <% //String realName=(String)request.getAttribute("realName");
                ...
                if((String)request.getAttribute("realName")==null){
                %>
    <jsp:forward page="/showInfoServlet"/>
        <%}
                int f;
                f=Integer.parseInt(request.getAttribute("f").toString());%>
    <div class="box">
    <form action="ChangeInfoServlet" method="post">
    <div class="form-group">
    <label>姓名</label>
    <input type="text" class="form-control" name="realName" id="realName"
    value="<%=(String)request.getAttribute("realName")%>"/>
    <div id="nickName-input-check"></div>
    </div>
    <div class="form-group">
    <label>性别</label>
    <select name="sex" id="sex" class="form-control">
        <%if(f==0){%><option value="男">男</option><%}%>
        <%if(f==1){%><option value="女">女</option><%}%>
    </select>
    <div id="sex-input-check">
    </div>
    </div>
    <div class="form-group">
    <label>生日</label>
    <div class="reRow">
    <input type="text" class="form-control col-3"  id="year" name="year"
    value="<%=(request.getAttribute("year"))%>" /><p>年</p>
    <input type="text" class="form-control col-3"  id="month" name="month"
    value="<%=request.getAttribute("month")%>" /><p>月</p>
    <input type="text" class="form-control col-3"  id="day" name="day"
    value="<%=(request.getAttribute("day"))%>" /><p>日</p>
    </div>
    ...
    value="<%=(String)request.getAttribute("addr")%>"/>
    ...
    <div class="form-group">
    <button class="btn btn-primary btn-block" name="reInfo-btn" id="reInfo-btn" type="submit">修改信息</button>
    </div>
    </form>
    </div>
    </div>
